...
export async function downloadPdf(r: Resume): Promise<void> {
  ...
  const headStack: any[] = [
    { text: t.name.upper ? (b.name || 'Your name').toUpperCase() : b.name || 'Your name', font: t.font, fontSize: t.name.size, bold: t.name.bold, color: t.ink, characterSpacing: -0.3, margin: [0, 0, 0, 2] },
  ];
  if (has(b.title)) headStack.push({ text: b.title, font: t.font, fontSize: t.role.size, italics: t.role.italic, bold: t.role.bold, color: colour(t.role.color), margin: [0, 0, 0, 4] });
  if (contactText.length) headStack.push({ text: contactText, font: prose, fontSize: t.contact.size, color: t.contact.color });
  ...
  if (t.headRule) content.push(line(t.headRule, 7, 13));
  else content.push({ canvas: [], margin: [0, 0, 0, 6] });

  // ---- sections
  const heading = (label: string) => {
    const out: any[] = [];
    if (t.h2.rule === 'above') out.push(line(t.rule, t.h2.top - 6, 4));
    out.push({
      text: t.h2.upper ? label.toUpperCase() : label,
      font: t.font, fontSize: t.h2.size, bold: t.h2.bold, color: colour(t.h2.color),
      margin: [0, t.h2.rule === 'above' ? 0 : t.h2.top - 6, 0, t.h2.rule === 'below' ? 2 : t.h2.bottom - 4],
      alignment: 'left',
    });
    if (t.h2.rule === 'below') out.push(line(t.rule, 0, t.h2.bottom - 4));
    return out;
  };
  ...
  const entry = (e: { title: string; org: string; dates: string; bullets: string[]; env: string }) => {
    const title = { text: e.title, font: t.font, fontSize: t.item.size, bold: t.item.bold, color: t.ink };
    const org = { text: e.org, font: prose, fontSize: t.fs, italics: t.item.orgItalic, bold: t.item.orgBold, color: colour(t.item.orgColor), margin: [0, 1, 0, 3] };
    ...
    if (e.bullets.filter(has).length) {
      body.push({ ul: e.bullets.filter(has).map((x) => ({ text: x, font: prose, fontSize: t.fs, color: t.ink })), markerColor: colour(t.bullet), margin: [0, 1, 0, 0] });
    }
    if (has(e.env)) body.push({ text: [{ text: 'Environment: ', bold: true }, { text: e.env }], font: prose, fontSize: t.fs - 0.7, color: t.mut, margin: [0, 2, 0, 0] });

    if (t.item.dates === 'gutter') {
      return { columns: [{ width: indent, text: e.dates, font: prose, fontSize: t.fs - 0.5, color: colour(t.item.datesColor) }, { width: '*', stack: [...head, ...body] }], columnGap: 0, margin: [0, 0, 0, 7], unbreakable: true };
    }
    return {
      stack: [
      ...
      margin: [0, 0, 0, 9], unbreakable: true,
    };
  };
  ...
  if (has(b.summary)) part.summary = [...heading('Professional Summary'), ...block([{ text: b.summary, font: prose, fontSize: t.fs, color: t.ink, lineHeight: t.lh, margin: [0, 0, 0, 0] }])];
  ...
  if (skills.length) {
    part.skills = [...heading('Skills'), ...block(skills.map((s) => ({
      ...
      font: prose, fontSize: t.fs, color: t.ink, lineHeight: t.lh, margin: [0, 0, 0, 2.5],
    })))];
  }
  ...
  if (has(r.languages)) part.languages = [...heading('Languages'), ...block([{ text: r.languages, font: prose, fontSize: t.fs, color: t.ink }])];
  ...
  const doc = {
    ...
    defaultStyle: { font: prose, fontSize: t.fs, lineHeight: t.lh, color: t.ink },
    content,
  };
